...
def calcular_utilizacao_look(utilizacoes, locais):
    ...
    if total_casa == 2 and total_mista == 1 and total == 3:
        return 'caseiro'

    if total_casa == 2 and total_sair == 1 and total == 3:
        return 'conforto'
...
def extrair_dados(arquivo_excel):
    ...
    campos_extra_idx = [11, 12, 13, 15, 16, 17, 18, 20, 21, 22, 23, 24, 25, 26, 27, 28, 72, 73]
    ...
    for row_idx, row in enumerate(ws_pecas.iter_rows(min_row=2, max_row=ws_pecas.max_row, values_only=True), start=2):
        try:
            ...
            no_lugar_de = valor_texto(row[72]) if len(row) > 72 else ''  # Coluna BU
            substituida_por = valor_texto(row[73]) if len(row) > 73 else ''  # Coluna BV
